# nokta35 — Blog Planı

> Topical Authority modeliyle yazılan ve planlanan tüm blog yazıları.
> ✅ = Yayında | 📝 = Yazılacak | 🔥 = Acil (tarihsel pencere kapanıyor)
>
> Her küme bir **Pillar** (ana sütun) etrafında toplanır.
> Cluster yazıları Pillar yazısına iç link atar.
>
> Son güncelleme: 2026-05-25 · **89 yayında** · 11 planlanan · Hedef: 100

---

## KÜME 1 — Hava Kalitesi & İklim

**Hedef:** Hava kirliliği, iklim değişikliği arayan kitleleri çek

| Tip | Durum | Başlık | Slug |
|-----|-------|--------|------|
| Pillar | ✅ | Hava Kalitesi Nedir? AKİ, PM2.5, NO₂ — Eksiksiz Rehber | `hava-kalitesi-rehberi` |
| Cluster | ✅ | Hava Kalitesi İndeksi (AKİ) Nedir? | `hava-kalitesi-indeksi-aki-nedir` |
| Cluster | ✅ | PM10 ve PM2.5 Nedir? Sağlığa Etkileri | `pm10-pm25-nedir` |
| Cluster | ✅ | Ozon (O₃) Kirliliği Nedir? Yüzey Ozonu ve Sağlık Etkileri | `ozon-kirliligi-nedir-saglik-etkileri` |
| Cluster | ✅ | Azot Dioksit (NO₂) Nedir? Trafik Kaynaklı Kirlilik | `azot-dioksit-no2-trafik-kirliligi` |
| Cluster | ✅ | Isı Adası Etkisi Nedir? Şehirlerde Sıcaklık Neden Yükselir? | `isi-adasi-etkisi-nedir` |
| Cluster | ✅ | İzmir Polen Takvimi: Hangi Ay, Hangi Bitki? | `izmir-polen-takvimi-alerji-mevsimi` |
| Cluster | 📝 | İklim Değişikliği ile Kuraklık Arasındaki Bağlantı | `iklim-degisikligi-kuraklik-baglantisi` |

**Durum:** 7/8 ✅ · 1 yazılacak

---

## KÜME 2 — Su & Baraj & Kuraklık

**Hedef:** Su güvenliği, kuraklık, baraj arayan kitleleri çek

| Tip | Durum | Başlık | Slug |
|-----|-------|--------|------|
| Pillar | ✅ | Kuraklık Nedir? Nasıl Ölçülür ve Sınıflandırılır? | `kuraklik-nedir-olculur` |
| Pillar | ✅ | Baraj Doluluk Oranı Nedir? Neden Önemli? | `baraj-doluluk-orani-nedir` |
| Cluster | ✅ | Türkiye'nin Önemli Barajları ve İşlevleri | `turkiye-onemli-barajlari` |
| Cluster | ✅ | Evde Su Tasarrufu: 12 Pratik Yöntem | `su-tasarrufu-yontemleri` |
| Cluster | ✅ | İzmir'in Barajları: Tahtalı, Gördes, Balçova, Kemer | `izmir-barajlari-tahtatli-gordes-balcova` |
| Cluster | ✅ | İzmir'de Su Kesintileri: Nedenler ve Ne Yapmalı? | `izmir-su-kesintileri-nedenler` |
| Cluster | ✅ | Su Sertliği Nedir? Sert Su Sağlığa Zararlı mı? | `su-sertligi-nedir-sagliga-zarari` |
| Cluster | 📝 | Yeraltı Suyu Nedir? Nasıl Oluşur, Nasıl Kullanılır? | `yeralti-suyu-nedir-nasil-olusur` |
| Cluster | 📝 | Yağmur Suyu Hasadı: Evde Yağmur Suyu Toplamak | `yagmur-suyu-hasadi-evde-toplama` |
| Cluster | ✅ | İçme Suyu Arıtma Yöntemleri: Hangisi Sizin İçin Doğru? | `icme-suyu-aritma-yontemleri` |

**Durum:** 8/10 ✅ · 2 yazılacak

---

## KÜME 3 — Sağlık & Eczane

**Hedef:** Sağlık hizmetleri arayan İzmirliler + genel kitle

| Tip | Durum | Başlık | Slug |
|-----|-------|--------|------|
| Pillar | ✅ | Türkiye'de Sağlık Sistemi: Birinci, İkinci, Üçüncü Basamak | `turkiye-saglik-sistemi-basamaklar` |
| Cluster | ✅ | Nöbetçi Eczane Sistemi Nasıl İşler? | `nobetci-eczane-sistemi` |
| Cluster | ✅ | Aile Sağlığı Merkezi (ASM) Nedir? Nasıl Kayıt Olunur? | `aile-sagligi-merkezi-asm-nedir` |
| Cluster | ✅ | Acil Servis mi, ASM mi? Doğru Tercih Kılavuzu | `acil-servis-mi-asm-mi` |
| Cluster | ✅ | İzmir Ücretsiz Psikolog ve Ruh Sağlığı Hizmetleri | `izmir-ucretsiz-psikolog-ruh-sagligi` |
| Cluster | ✅ | İzmir Bayram Nöbetçi Eczane 2026 | `izmir-bayram-nobetci-eczane-2026` |
| Cluster | ✅ | SGK ile Hangi İlaçlar Karşılanır? Katkı Payı Rehberi | `sgk-ilac-katki-payi-rehberi` |
| Cluster | ✅ | Reçetesiz (OTC) İlaçlar Nelerdir? Eczanede Nasıl Alınır? | `recetesiz-otc-ilaclar-nedir` |
| Cluster | 📝 | Engelli Sağlık Kurulu Raporu Nasıl Alınır? | `engelli-saglik-kurulu-raporu-nasil-alinir` |

**Durum:** 8/9 ✅ · 1 yazılacak

---

## KÜME 4 — Ulaşım (İzmir Odaklı)

**Hedef:** İzmir'de ulaşım arayan yerel kitle + şehre yeni gelenler

| Tip | Durum | Başlık | Slug |
|-----|-------|--------|------|
| Pillar | ✅ | İzmir Toplu Taşıma Rehberi: ESHOT, İZBAN, Metro, Vapur | `izmir-toplu-tasima-rehberi` |
| Cluster | ✅ | İzmir Otobüs Hatları Nasıl Numaralandırılır? | `otobus-hat-numaralandirma` |
| Cluster | ✅ | İZBAN Nedir? İstasyonlar ve Güzergah Haritası | `izban-nedir-istasyonlar-guzergah` |
| Cluster | ✅ | İzmir Metrosu: Güzergahlar, İstasyonlar, Ücretler | `izmir-metrosu-guzergahlar-istasyonlar` |
| Cluster | ✅ | İZMİRKART Nedir? Nasıl Yüklenir ve Kullanılır? | `izmirkart-nedir-nasil-kullanilir` |
| Cluster | ✅ | BİSİM Bisiklet İzmir: Nasıl Kullanılır, İstasyonlar | `bisim-bisiklet-izmir-kullanim-rehberi` |
| Cluster | ✅ | İzmir Bayram Ulaşımı 2026: İZBAN, ESHOT, İZDENİZ | `izmir-bayram-ulasimi-2026` |
| Cluster | ✅ | İzmir Otopark: İZELMAN Tesisleri Rehberi | `izmir-otopark-izelaman-rehberi` |
| Cluster | ✅ | Adnan Menderes Havalimanı'ndan Şehir Merkezine Ulaşım | `izmir-adnan-menderes-havalimani-ulasim` |
| Cluster | ✅ | İzmir'de Ücretsiz ve Ucuz Park Yerleri | `izmir-ucretsiz-park-yerleri` |
| Cluster | ✅ | İzmir Vapurları: Güzergahlar, İZDENİZ Sefer Saatleri | `izmir-vapurlari-guzergahlar-seferler` |
| Cluster | 📝 | İzmir'de Araç Kiralama vs. Toplu Taşıma: Maliyet | `izmir-arac-kiralama-toplu-tasima` |

**Durum:** 11/12 ✅ · 1 yazılacak

---

## KÜME 5 — Açık Veri & Teknoloji

**Hedef:** Sivil teknoloji, veri gazeteciliği, geliştiriciler

| Tip | Durum | Başlık | Slug |
|-----|-------|--------|------|
| Pillar | ✅ | Açık Veri Nedir? Şehirler İçin Neden Önemli? | `acik-veri-nedir` |
| Cluster | ✅ | API Nedir? Açık Veri ile Farkı Ne? | `api-nedir-acik-veri-farki` |
| Cluster | ✅ | JSON, CSV, XML: Veri Formatları Arasındaki Fark | `json-csv-xml-veri-formatlari-farki` |
| Cluster | ✅ | Akıllı Şehir (Smart City) Nedir? Temel Kavramlar | `akilli-sehir-smart-city-nedir` |
| Cluster | ✅ | Coğrafi Bilgi Sistemi (CBS/GIS) Nedir? | `cografi-bilgi-sistemi-cbs-gis-nedir` |
| Cluster | ✅ | Belediye Açık Verisi: Türkiye'de ve Dünyada Durum | `belediye-acik-verisi-durum` |

**Durum:** 6/6 ✅ · Küme tamamlandı

---

## KÜME 6 — İzmir Genel (Yerel + Turist Kitlesi)

**Hedef:** "İzmir" araması yapan herkes — en geniş kitleyi kapsar

| Tip | Durum | Başlık | Slug |
|-----|-------|--------|------|
| Pillar | ✅ | İzmir Rehberi: Coğrafya, İklim, İlçeler, Ulaşım | `izmir-rehberi-cografya-iklim-ilceler` |
| Cluster | ✅ | İzmir'de Pazar Yerleri: Hangi Gün, Hangi İlçede? | `izmir-pazar-yerleri-rehberi` |
| Cluster | ✅ | İzmir'de Deprem Riski: Fay Hatları ve Hazırlık | `izmir-deprem-riski-fay-hatlari` |
| Cluster | ✅ | İzmir Kent Lokantaları ve Halk Ekmek Büfeleri | `izmir-kent-lokantalari-halk-ekmek` |
| Cluster | ✅ | İzmir Ücretsiz Etkinlik Alanları | `izmir-ucretsiz-etkinlik-alanlari` |
| Cluster | ✅ | İzmir'de Geç Saatlere Kadar Açık Kütüphaneler | `izmir-7-24-acik-kutuphaneler` |
| Cluster | ✅ | İzmir Ücretsiz Spor Tesisleri ve Yürüyüş Parkurları | `izmir-ucretsiz-spor-tesisleri-parkurlar` |
| Cluster | ✅ | Gediz Elektrik Kesintisi Sorgulama: SMS, Uygulama, 186 | `gediz-elektrik-kesintisi-sorgulama` |
| Cluster | ✅ | İzmir Lise Taban Puanları: LGS Sistemi ve Resmi Sorgulama | `izmir-lise-taban-puanlari-lgs-rehberi` |
| Cluster | ✅ | İzmir Kira Fiyatları: Araştırma Yöntemleri | `izmir-kira-fiyatlari-nasil-arastirilir` |
| Cluster | ✅ | İzmir Zemin Sıvılaşma Riski | `izmir-zemin-sivilasma-riski` |
| Cluster | ✅ | İzmir'in İlçeleri: Kaç İlçe Var, Hangisi Nerede? | `izmir-ilceleri-kac-ilce-var` |
| Cluster | ✅ | İzmir'de Hava Nasıl? İklim Özellikleri ve Mevsimler | `izmir-hava-iklim-ozellikleri` |
| Cluster | 📝 | İzmir'de Yaşam Maliyeti: 2026 Güncel Rehber | `izmir-yasam-maliyeti-2026` |
| Cluster | 📝 | İzmir Tabiat Koruma Alanları: Doğa Yürüyüşü Rotaları | `izmir-tabiat-koruma-alanlari-doga-yuruyusu` |

**Durum:** 13/15 ✅ · 2 yazılacak

---

## KÜME 7 — Sürdürülebilir Yaşam

**Hedef:** Sürdürülebilirlik, çevre, sıfır atık arayanlar — ulusal geniş kitle

| Tip | Durum | Başlık | Slug |
|-----|-------|--------|------|
| Pillar | ✅ | Sürdürülebilir Yaşam Nedir? Evden Başlamak İçin Kılavuz | `surdurulebilir-yasam-nedir-rehber` |
| Cluster | ✅ | Sıfır Atık Yaşam: Başlangıç için 10 Adım | `sifir-atik-yasam-baslangic` |
| Cluster | ✅ | İzmir Geri Dönüşüm Noktaları ve Elektronik Atık Rehberi | `izmir-geri-donusum-elektronik-atik` |
| Cluster | ✅ | Gıda İsrafını Önlemek: 12 Pratik Yöntem | `gida-israfi-onlemek-pratik-yontemler` |
| Cluster | ✅ | Kompost Nedir? Evde Kompost Nasıl Yapılır? | `kompost-nedir-evde-nasil-yapilir` |
| Cluster | ✅ | Organik Tarım Nedir? Konvansiyonel Tarımdan Farkı | `organik-tarim-nedir-farki` |
| Cluster | ✅ | Yenilenebilir Enerji Türleri: Güneş, Rüzgar, Jeotermal | `yenilenebilir-enerji-turleri` |
| Cluster | ✅ | Evde Solar Panel: Kurulum, Maliyet, Geri Dönüş Rehberi | `evde-solar-panel-kurulum-maliyet` |
| Cluster | ✅ | Karbon Ayak İzi Nedir? Nasıl Hesaplanır? | `karbon-ayak-izi-nedir-hesaplanir` |

**Durum:** 9/9 ✅ · Küme tamamlandı

---

## KÜME 8 — Afet Hazırlığı & Kentsel Güvenlik

**Hedef:** Afet farkındalığı yüksek kitle — İzmir'e özgü (2020 depremi belleği)

| Tip | Durum | Başlık | Slug |
|-----|-------|--------|------|
| Pillar | ✅ | Depreme Hazırlık: Ev, Çanta, Tahliye Planı | `depreme-hazirlik-rehberi` |
| Cluster | ✅ | Deprem Çantası: İçinde Ne Olmalı? (Güncel Liste) | `deprem-cantasi-icinde-ne-olmali` |
| Cluster | ✅ | AFAD Nedir? Afetlerde Rolü ve İletişim Bilgileri | `afad-nedir-afetlerde-rolu` |
| Cluster | ✅ | Toplanma Alanı Nedir? Nasıl Bulunur? | `toplanma-alani-nedir-nasil-bulunur` |
| Cluster | ✅ | İzmir Zemin Sıvılaşma Riski: Hangi İlçeler Tehlike Altında? | `izmir-zemin-sivilasma-riski` |
| Cluster | ✅ | Sel ve Taşkın: Öncesinde ve Sırasında Ne Yapmalı? | `sel-taskin-oncesi-sirasinda-ne-yapmali` |
| Cluster | 📝 | Tsunami Riski: Ege Kıyılarında Tarihsel Durum | `tsunami-riski-ege-kiyilari` |

**Durum:** 6/7 ✅ · 1 yazılacak

> Not: İzmir zemin sıvılaşması hem Küme 6 hem Küme 8'de geçiyor; index.ts'de tek kayıt.

---

## KÜME 9 — Şehir & Kentsel Yaşam

**Hedef:** Şehir konuları arayan geniş kitle — Türkiye geneli

| Tip | Durum | Başlık | Slug |
|-----|-------|--------|------|
| Pillar | ✅ | Şehir Planlaması Nedir? İmar, Kentsel Rant ve Dönüşüm | `sehir-planlamasi-nedir-imar-rant` |
| Cluster | ✅ | Kentsel Dönüşüm Nedir? Hak ve Yükümlülükler | `kentsel-donusum-nedir-hak-yukumlulukler` |
| Cluster | ✅ | Büyükşehir Belediyesi ile İlçe Belediyesi Farkı | `buyuksehir-ilce-belediyesi-farki` |
| Cluster | ✅ | İmar Durumu Nedir? Nasıl Sorgulanır ve Yorumlanır? | `imar-durumu-nedir-nasil-sorgulanir` |
| Cluster | ✅ | Ulaşım Ana Planı Nedir? Şehirlerde Nasıl Hazırlanır? | `ulasim-ana-plani-nedir` |

**Durum:** 5/5 ✅ · Küme tamamlandı

---

## KÜME 10 — Sezonluk & Güncel İçerik 🔥

**Hedef:** Mevsimsel arama dalgalarını yakala — sınav, tatil, bayram, oran güncellemeleri

| Tip | Durum | Başlık | Slug |
|-----|-------|--------|------|
| Sezon | ✅ | İzmir Kurban Kesim Yerleri 2026: Mezbaha ve Geçici Alanlar | `izmir-kurban-kesim-yerleri-2026` |
| Sezon | ✅ | İzmir Bayram Ulaşımı 2026: İZBAN, ESHOT, İZDENİZ | `izmir-bayram-ulasimi-2026` |
| Sezon | ✅ | Bayram Namazı Saat Kaçta İzmir — 27 Mayıs 2026 | `bayram-namazi-saati-izmir-2026` |
| Sezon | ✅ | İzmir Bayram Nöbetçi Eczane 2026 | `izmir-bayram-nobetci-eczane-2026` |
| Sezon | ✅ | Bayramda Bankalar, PTT ve AVM'ler Açık mı? İzmir 2026 | `bayramda-bankalar-ptt-avm-izmir-2026` |
| Sezon | ✅ | Kurban Derisi Nereye Verilir? İzmir 2026 | `kurban-derisi-nereye-verilir-izmir-2026` |
| Sezon | ✅ | Mayıs 2026 Kira Artış Oranı: Yasal Tavan ve Hesaplama | `mayis-2026-kira-artis-orani` |
| Sezon | ✅ | İzmir Plajları Mavi Bayrak 2026: Tam Liste (60 Plaj) | `izmir-plajlari-mavi-bayrak-2026` |
| Sezon | ✅ | İzmir Deniz Suyu Sıcaklığı: Aylık Ortalama Takvim | `izmir-deniz-suyu-sicakligi-aylik` |
| Sezon | ✅ | LGS 2026 İzmir: Sınav Günü Ulaşım ve Hazırlık Rehberi | `lgs-2026-izmir-sinav-ulasim-rehberi` |
| Sezon | ✅ | YKS 2026 İzmir: Sınav Merkezleri ve Ulaşım | `yks-2026-izmir-sinav-merkezleri` |
| Sezon | 📝 | Haziran 2026 Kira Artış Oranı | `haziran-2026-kira-artis-orani` |
| Sezon | 📝 | Temmuz 2026 Kira Artış Oranı | `temmuz-2026-kira-artis-orani` |

**Durum:** 11/13 ✅ · 2 yazılacak

---

## KÜME 11 — Resmi İşlemler & Günlük Hayat (YENİ KÜME)

**Hedef:** "nasıl yapılır" + "nedir" aramaları — KGR < 0.22, evergreen, düşük rekabet

Bu kümede "nereden, nasıl" tipi yüksek niyetli (yüksek dönüşüm) aramaları yakalarız.
Resmi kurum adımları içerdiğinden doğruluğu kritik; uydurma bilgi yasak.

| Tip | Durum | Başlık | Slug |
|-----|-------|--------|------|
| Pillar | ✅ | e-Devlet ile Neler Yapılabilir? Başvurudan Belgeye Rehber | `e-devlet-ne-yapilir-rehber` |
| Cluster | ✅ | İkametgah Belgesi Nasıl Alınır? (e-Devlet ve Nüfus Müdürlüğü) | `ikametgah-belgesi-nasil-alinir` |
| Cluster | ✅ | SGK Hizmet Dökümü e-Devlet'ten Nasıl Alınır? | `sgk-hizmet-dokumu-e-devlet` |
| Cluster | ✅ | KEP Adresi Nedir? Nasıl Alınır? | `kep-adresi-nedir-nasil-alinir` |
| Cluster | ✅ | Elektrik Sayacı Endeks Bildirimi Nasıl Yapılır? | `elektrik-sayaci-endeks-bildirimi` |
| Cluster | ✅ | Asansör Bakımı: Kim Sorumlu, Nasıl Şikâyet Edilir? | `asansor-bakimi-kim-sorumlu` |
| Cluster | ✅ | Adres Değişikliği Bildirimi: Ne Zaman, Nasıl? | `adres-degisikligi-bildirimi` |
| Cluster | ✅ | Tapu Sorgusu e-Devlet'ten Nasıl Yapılır? | `tapu-sorgusu-e-devlet` |
| Cluster | 📝 | Veraset İlamı Nedir? Nasıl Alınır? | `veraset-ilami-nedir-nasil-alinir` |

**Durum:** 8/9 ✅ · 1 yazılacak

---

## ÖZET TABLO

| Küme | Tamamlanan | Planlanan | Toplam |
|------|-----------|-----------|--------|
| 1 — Hava Kalitesi | 7 | 1 | 8 |
| 2 — Su & Baraj | 8 | 2 | 10 |
| 3 — Sağlık | 8 | 1 | 9 |
| 4 — Ulaşım | 11 | 1 | 12 |
| 5 — Açık Veri | 6 | 0 | 6 |
| 6 — İzmir Genel | 13 | 2 | 15 |
| 7 — Sürdürülebilir Yaşam | 9 | 0 | 9 |
| 8 — Afet Hazırlığı | 6 | 1 | 7 |
| 9 — Şehir & Kentsel | 5 | 0 | 5 |
| 10 — Sezonluk & Güncel | 11 | 2 | 13 |
| 11 — Resmi İşlemler | 8 | 1 | 9 |
| **Toplam** | **89** | **11** | **100** |

---

## ÖNCELİK SIRASı

### 🔥 ACİL — Haziran Başına Kadar (Tarihsel pencere kapanıyor)

| Sıra | Başlık | Son Tarih | Neden Acil |
|------|--------|-----------|------------|
| 1 | İzmir Plajları Mavi Bayrak 2026 | 1 Haz | Yaz sezonu, şu an arama zirve |
| 2 | İzmir Deniz Suyu Sıcaklığı | 5 Haz | Yaz tatili planlaması başladı |
| 3 | LGS 2026 İzmir Ulaşım Rehberi | 10 Haz | Sınav: 13 Haziran Cumartesi |
| 4 | YKS 2026 İzmir Sınav Merkezleri | 15 Haz | Sınav: 20–21 Haziran |

### ⚡ YÜKSEK ÖNCELİK — KGR < 0.22, Evergreen

| Sıra | Başlık | KGR | Küme | Durum |
|------|--------|-----|------|-------|
| 5 | Adnan Menderes Havalimanı'ndan Şehir Merkezine Ulaşım | ~0.15 | 4 | ✅ |
| 6 | İkametgah Belgesi Nasıl Alınır? | ~0.18 | 11 | ✅ |
| 7 | e-Devlet ile Neler Yapılabilir? (Pillar) | ~0.20 | 11 | ✅ |
| 8 | SGK Hizmet Dökümü e-Devlet'ten Nasıl Alınır? | ~0.22 | 11 | ✅ |
| 9 | KEP Adresi Nedir? Nasıl Alınır? | ~0.15 | 11 | ✅ |
| 10 | Elektrik Sayacı Endeks Bildirimi | ~0.20 | 11 | ✅ |
| 11 | Asansör Bakımı: Kim Sorumlu? | ~0.20 | 11 | ✅ |
| 12 | İzmir'de Ücretsiz ve Ucuz Park Yerleri | ~0.22 | 4 | ✅ |

### 📋 ORTA ÖNCELİK — Mevcut Kümelerin Tamamlanması

| Sıra | Başlık | Küme |
|------|--------|------|
| 13 | İzmir Polen Takvimi: Hangi Ay, Hangi Bitki? | 1 |
| 14 | İzmir İlçeleri: Kaç İlçe Var, Hangisi Nerede? | 6 |
| 15 | İzmir'de Hava Nasıl? İklim Özellikleri ve Mevsimler | 6 |
| 16 | Su Sertliği Nedir? Sert Su Sağlığa Zararlı mı? | 2 |
| 17 | İçme Suyu Arıtma Yöntemleri | 2 |
| 18 | SGK İlaç Katkı Payı Rehberi | 3 |
| 19 | Reçetesiz (OTC) İlaçlar Nelerdir? | 3 |
| 20 | Sel ve Taşkın: Öncesinde ve Sırasında Ne Yapmalı? | 8 |
| 21 | İzmir Vapurları: Güzergahlar, İZDENİZ Sefer Saatleri | 4 |
| 22 | Adres Değişikliği Bildirimi | 11 |
| 23 | Tapu Sorgusu e-Devlet'ten Nasıl Yapılır? | 11 |

### 🗓️ DÜŞÜKÖNCELİK — Araştırma / Hassas Veri Gerektirir

- İzmir Yaşam Maliyeti 2026 (anlık fiyat verisi riski)
- Kira artış oranı aylık güncellemeler (Haz, Tem, Ağu)
- Haziran/Temmuz 2026 kira artış tavanı (TÜİK açıklamasından sonra)

---

## YAZARKEN KURALLLAR

1. **Uydurma sayı yasak.** Lise taban puanı, kira fiyatı, otopark ücreti, ilaç fiyatı gibi değişken sayılar verilmez; metodoloji rehberi verilir, resmi kaynak gösterilir.
2. **Her Cluster kendi Pillar'ına en az 1 iç link vermeli.**
3. **Her yazıda 5 FAQ** (schema.org FAQPage için zorunlu).
4. **KGR kontrolü:** allintitle:["anahtar kelime"] / aylık arama hacmi < 0.25 ideal.
5. **Tarihli içerik** (2026 bayram, LGS, YKS): Slug'a yıl ekle, slug değiştirilebilir URL olarak kullan; her yıl yeni yazı açılır veya mevcut `dateModified` güncellenir.
6. **Küme 11 özel dikkat:** Resmi işlemler yanlış yönlendirmeye kapalı olmalı; belirsiz adımlarda "güncel bilgi için [kurum adı] resmi sitesini kontrol edin" notu zorunlu.
